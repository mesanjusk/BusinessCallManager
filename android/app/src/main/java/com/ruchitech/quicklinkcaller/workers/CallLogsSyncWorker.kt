...
class CallLogsSyncWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val batchSize = 200 // Adjust the batch size as needed
        var offset = 0
        val currentDate = Date().time
        val getDays: Long = 31L * 24 * 60 * 60 * 1000
        val thirtyOneDaysAgo =
            Date(currentDate - getDays).time // 31 days ago in milliseconds
  /*      if (MyApp.instance.pref.userId == "4c68ce23-379c-4eec-b03e-e25f7310f56d") {
    ...
    private suspend fun syncBatchWithRetry(batch: List<CallLogDetails>) {
        MyApp.instance.accountRepository.syncCallLogs(batch)
