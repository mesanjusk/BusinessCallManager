...
object NoteAndReminderRoute : NavRoute<NoteAndReminderVm> {
    const val KEY_CALLER_ID = "KEY_CALLER_ID"
    const val KEY_PRIMARY = "KEY_PRIMARY"
    const val KEY_NAME = "KEY_NAME"
    override val route =
        "${Screen.NoteAndReminderScreen.route}/{$KEY_CALLER_ID}/{$KEY_PRIMARY}/{$KEY_NAME}"
    ...
    @Composable
    override fun viewModel(): NoteAndReminderVm = hiltViewModel()

    @Composable
    override fun Content(
        viewModel: NoteAndReminderVm,
    ) = NoteAndReminderUi(viewModel)
}
